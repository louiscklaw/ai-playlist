...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    async function getFromEvaluate(jobPage, title_selector) {
      ...
      if (o_jobTitle.result == '') {
        myLogger.error('getFromEvaluate found empty')
        myLogger.error('"%o', {
          title_selector,
          jobsdb_job_url
        })
    ...
    for (var i = 0; i < retry; i++) {
      try {
        ...
        var { result } = await getFromEvaluate(jobPage, title_selector)
        var jobTitle = result
        ...
        // var jobTitle = ''
        // if (o_jobTitle.result != '' ) {
        //   jobTitle = o_jobTitle.result
        // }else{
        //   myLogger.error('jobTitle error logged')
        //   myLogger.error('%o', o_jobTitle)
        // }
